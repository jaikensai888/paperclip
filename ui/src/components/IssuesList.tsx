...
const quickFilterPresets = (t: (key: string) => string) => [
  { label: t('issues.all'), statuses: [] as string[] },
  { label: t('issues.active'), statuses: ["todo", "in_progress", "in_review", "blocked"] },
  { label: t('issues.backlog'), statuses: ["backlog"] },
  { label: t('issues.done'), statuses: ["done", "cancelled"] },
];
      ...
      label:
        key === "__unassigned"
          ...
          : key.startsWith("__user:")
            ? (formatAssigneeUserLabel(key.slice("__user:".length), currentUserId) ?? t('issues.me'))
            : (agentName(key) ?? key.slice(0, 8)),
      items: groups[key]!,
    }));
  }, [filtered, viewState.groupBy, agents, agentName, currentUserId, t]);
          ...
          <Popover>
            <PopoverTrigger asChild>
              <Button variant="ghost" size="sm" className={`text-xs ${activeFilterCount > 0 ? "text-blue-600 dark:text-blue-400" : ""}`}>
                ...
                <span className="hidden sm:inline">{activeFilterCount > 0 ? `${t('issues.filters')}: ${activeFilterCount}` : t('issues.filter')}</span>
                {activeFilterCount > 0 && (
              ...
              <div className="p-3 space-y-3">
                ...
                <div className="space-y-1.5">
                  ...
                  <div className="flex flex-wrap gap-1.5">
                    {quickFilterPresets(t).map((preset) => {
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-x-4 gap-y-3">
                  ...
                  <div className="space-y-3">
                    ...
                    <div className="space-y-1">
                      ...
                      <div className="space-y-0.5 max-h-32 overflow-y-auto">
                        <label className="flex items-center gap-2 px-2 py-1 rounded-sm hover:bg-accent/50 cursor-pointer">
                          ...
                          />
                          <span className="text-sm">{t('issues.noAssignee')}</span>
                        </label>
                        {currentUserId && (
                          <label className="flex items-center gap-2 px-2 py-1 rounded-sm hover:bg-accent/50 cursor-pointer">
                            <Checkbox
                            ...
                            <span className="text-sm">{t('issues.me')}</span>
                          </label>
                        )}
                        {(agents ?? []).map((agent) => (
                    ...
                    {labels && labels.length > 0 && (
                      <div className="space-y-1">
                        <span className="text-xs text-muted-foreground">{t('issues.labels')}</span>
                        <div className="space-y-0.5 max-h-32 overflow-y-auto">
                          {labels.map((label) => (
                          ...
                          >
                            {issue.assigneeAgentId && agentName(issue.assigneeAgentId) ? (
                              ...
                              <span className="inline-flex items-center gap-1.5 text-xs">
                                ...
                                {formatAssigneeUserLabel(issue.assigneeUserId, currentUserId) ?? t('issues.me')}
                              </span>
                            ) : (
