...
export function NewProjectDialog() {
  ...
  const projectStatuses = [
    { value: "backlog", label: t('projectStatuses.backlog') },
    { value: "planned", label: t('projectStatuses.planned') },
    { value: "in_progress", label: t('projectStatuses.in_progress') },
  ...
  const deriveWorkspaceNameFromPath = (value: string) => {
    ...
    return segments[segments.length - 1] ?? t('dialogs.newProject.localFolderDefault');
  };
  ...
  const deriveWorkspaceNameFromRepo = (value: string) => {
    try {
      ...
      return repo || t('dialogs.newProject.githubRepoDefault');
    } catch {
      return t('dialogs.newProject.githubRepoDefault');
    }
  };
  ...
  async function handleSubmit() {
    ...
    if (localRequired && !isAbsolutePath(localPath)) {
      setWorkspaceError(t('dialogs.newProject.localFolderError'));
      return;
    }
    if (repoRequired && !isGitHubRepoUrl(repoUrl)) {
      setWorkspaceError(t('dialogs.newProject.repoUrlError'));
      return;
    }
      ...
      >
        ...
        <div className="flex items-center justify-between px-4 py-2.5 border-b border-border">
          <div className="flex items-center gap-2 text-sm text-muted-foreground">
            ...
            <span>{t('dialogs.newProject.title')}</span>
          </div>
        ...
        <div className="px-4 pt-4 pb-2 shrink-0">
          <input
            ...
            placeholder={t('dialogs.newProject.projectName')}
            value={name}
        ...
        <div className="px-4 pb-2">
          <MarkdownEditor
            ...
            placeholder={t('dialogs.newProject.addDescription')}
            bordered={false}
        ...
        <div className="px-4 pb-3 space-y-3 border-t border-border">
          <div className="pt-3">
            <p className="text-sm font-medium">{t('dialogs.newProject.workspaceTitle')}</p>
            <p className="text-xs text-muted-foreground">{t('dialogs.newProject.workspaceHint')}</p>
          </div>
          <div className="grid gap-2 sm:grid-cols-3">
            ...
            >
              <div className="flex items-center gap-2 text-sm font-medium">
                ...
                {t('dialogs.newProject.localFolder')}
              </div>
              <p className="mt-1 text-xs text-muted-foreground">{t('dialogs.newProject.localFolderHint')}</p>
            </button>
            <button
              type="button"
              className={cn(
            ...
            >
              <div className="flex items-center gap-2 text-sm font-medium">
                ...
                {t('dialogs.newProject.githubRepo')}
              </div>
              <p className="mt-1 text-xs text-muted-foreground">{t('dialogs.newProject.githubRepoHint')}</p>
            </button>
            <button
              type="button"
              className={cn(
            ...
            >
              <div className="flex items-center gap-2 text-sm font-medium">
                ...
                {t('dialogs.newProject.both')}
              </div>
              <p className="mt-1 text-xs text-muted-foreground">{t('dialogs.newProject.bothHint')}</p>
            </button>
          </div>

          {(workspaceSetup === "local" || workspaceSetup === "both") && (
            <div className="rounded-md border border-border p-2">
              <label className="mb-1 block text-xs text-muted-foreground">{t('dialogs.newProject.localFolderPath')}</label>
          ...
          {(workspaceSetup === "repo" || workspaceSetup === "both") && (
            <div className="rounded-md border border-border p-2">
              <label className="mb-1 block text-xs text-muted-foreground">{t('dialogs.newProject.githubRepoUrl')}</label>
              <input
        ...
        <div className="flex items-center gap-1.5 px-4 py-2 border-t border-border flex-wrap">
          ...
          <Popover open={goalOpen} onOpenChange={setGoalOpen}>
            <PopoverTrigger asChild>
              ...
              >
                ...
                {selectedGoals.length > 0 ? t('dialogs.newProject.addGoal') : t('dialogs.newProject.goal')}
              </button>
            </PopoverTrigger>
            <PopoverContent className="w-56 p-1" align="start">
              ...
              {selectedGoals.length > 0 && availableGoals.length === 0 && (
                <div className="px-2 py-1.5 text-xs text-muted-foreground">
                  {t('dialogs.newProject.allGoalsSelected')}
                </div>
              )}
            </PopoverContent>
          </Popover>
          ...
          <div className="inline-flex items-center gap-1.5 rounded-md border border-border px-2 py-1 text-xs">
            ...
            <input
              ...
              placeholder={t('dialogs.newProject.targetDate')}
            />
          </div>
        </div>
        ...
        <div className="flex items-center justify-between px-4 py-2.5 border-t border-border">
          {createProject.isError ? (
            <p className="text-xs text-destructive">{t('dialogs.newProject.failedToCreate')}</p>
          ) : (
            <span />
          )}
          <Button
          ...
          >
            {createProject.isPending ? t('dialogs.newProject.creating') : t('dialogs.newProject.createProject')}
          </Button>
        </div>
      </DialogContent>
    </Dialog>
