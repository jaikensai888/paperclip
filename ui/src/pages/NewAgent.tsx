  ...
  function handleSubmit() {
    ...
    if (configValues.adapterType === "opencode_local") {
      ...
      if (adapterModelsLoading || adapterModelsFetching) {
        setFormError(t("newAgent.openCodeModelsLoading"));
        return;
      }
      ...
      if (!discovered.some((entry) => entry.id === selectedModel)) {
        setFormError(
          discovered.length === 0
            ? t("newAgent.noOpenCodeModels")
            : t("newAgent.openCodeModelUnavailable", { model: selectedModel }),
        );
        return;
      }
    }
              ...
              >
                {currentReportsTo ? (
                  <>
                    ...
                    {t("newAgent.reportsTo", { name: currentReportsTo.name })}
                  </>
                ) : (
                  <>
                    <User className="h-3 w-3 text-muted-foreground" />
                    {isFirstAgent ? t("newAgent.reportsToNA") : t("newAgent.reportsToSelect")}
                  </>
                )}
              </button>
            </PopoverTrigger>
