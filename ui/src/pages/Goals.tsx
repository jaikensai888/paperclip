...
export function Goals() {
  ...
  const handleStatusChange = (goal: Goal, status: string) => {
    updateGoalStatus.mutate({ goalId: goal.id, status });
  };
    ...
    <div className="space-y-4">
      ...
      {goals && goals.length > 0 && (
        <>
          ...
          <GoalTree goals={goals} goalLink={(goal) => `/goals/${goal.id}`} onStatusChange={handleStatusChange} />
        </>
      )}
    </div>
  );
